...
*** Respuesta
   #+BEGIN_COMMENT
   <<DUDA>>: punto 4
   (4) el scanner realiza 7 invocaciones a ~ungetc~, la secuencia de caracteres devueltos por ~ungetc~ es
   1. +
   2. F
   3. +
   4. 0
   ...
   1. ~0xF~ (constante entera hexadecimal)
   2. ~+~ (operador)
   3. ~F0x~ (identificador) <- empiezan con una letra o guión bajo, seguido ó no de numeros,letras,guiones en cualquier orden
   4. ~+~ (operador)
   5. ~0L~ (constante entera)
   6. ~+~ (operador)
   7. ~L0~ (identificador, por empezar con una letra)

   (3) el scanner hace 20 invocaciones a ~getchar~
   - una invocación por cada caracter leído,
   - dos invocaciones a ~getchar~ si el siguiente caracter no pertenece al TOKEN del anterior
     porque hace ~ungetc~ para retroceder y volver a leer ese caracter
   - la secuencia de lectura para ~0xF+F0x+0L+L0~ sería
     1) 0xF+ (4 lecturas, e invoca a ~ungetc~ porque ~+~ no pertenece al tipo de TOKEN identificadores)
     2) +F (2 lecturas, e invoca a ~ungetc~ porque ~F~ no pertenece al tipo de TOKEN operadores)
     3) F0x+ (4 lecturas, ungetc por +)
     4) +0 (2 lecturas, ungetc por 0)
     5) 0L+ (3 lecturas, ungetc por +)
     6) +L (2lecturas, ungetc por L)
     7) L0 (2 lecturas)
   - Si sumamos la cant. de invocaciones a ~getchar~ nos da 4+2+4+2+3+2+2 = 20
...
*** Respuesta
   ...
   alternativa 1? <-- creo que esta?
   1. if( <-- ungetc por el ~(~
   2. (0 <-- ungetc por el ~0~
   3. 0) <-- ungetc por el ~)~
   4. )r <-- ungetc por la ~r~
   5. return 1; <--- ungetc por ~;~
   6. return 42; <-- ungetc por el ~;~
   ...
   1. ~4~ (primera invocación, lee la constante 4)
   2. ~2~ (segunda invocación)
   3. ~;~ (tercera invocación), cuando detecta el caracter de puntuación ~;~ sabe que terminó la secuencia

   (2) Que el parámetro con identificador ~x~ no se está utilizando, en el cuerpo de la funcion
   
   (3) Que el comportamiento de ~f~ es retornar siempre 42, ya que la condición de if nunca se cumple
   al pasarle ~0~
   #+END_QUOTE
